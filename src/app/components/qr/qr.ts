...
export class Qr {
  isModalOpen = false;
  selectedProfile: QrProfile | null = null;
    ...
    this.profiles = [
      {
        ...
        metrics: [
          { label: 'Network', value: '500+ Connections', icon: '🔗' },
          { label: 'Domain', value: 'Salesforce Ecosystem', icon: '☁️' },
          { label: 'Hub', value: 'TCS Jaipur Tech Hub', icon: '🏙️' },
        ],
      ...
      },
      {
        ...
        metrics: [
          { label: 'Rank', value: 'Ranger', icon: '🏆' },
          { label: 'Credentials', value: '100+ Badges', icon: '🎖️' },
          { label: 'Practice', value: 'Cloud Practice Hub', icon: '⚡' },
        ],
      ...
      },
      {
        ...
        metrics: [
          { label: 'Repositories', value: 'Public Projects', icon: '📦' },
          { label: 'Activity', value: 'Active Commits', icon: '✅' },
          { label: 'Engineering', value: 'Developer Unit', icon: '💻' },
        ],
  ...
  private qrUrl(targetUrl: string, fgColor: string): string {
    ...
    return `https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=${encoded}&color=${fgColor}&bgcolor=ffffff&qzone=1&margin=0&format=png`;
  }

  openMetrics(profile: QrProfile): void {
